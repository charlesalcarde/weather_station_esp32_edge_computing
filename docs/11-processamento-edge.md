...
Após calcular características, o firmware converte valores contínuos em estados discretos.

De forma geral, para uma variável $x$ e limiares $L_1$ e $L_2$, a classificação pode ser descrita por três condições independentes:

$$
x<L_1 \Rightarrow C(x)=C_1
$$

$$
L_1\le x<L_2 \Rightarrow C(x)=C_2
$$

$$
x\ge L_2 \Rightarrow C(x)=C_3
$$

onde $C_1$, $C_2$ e $C_3$ representam as classes ambientais definidas pelo firmware.

No firmware real, os estados e limiares devem corresponder às regras implementadas.

A vantagem da classificação é transformar números em informação operacional.
...
Se $t_c$ é o tempo durante o qual uma nova condição permaneceu válida e $T_H$ é o tempo mínimo de confirmação:
...
A regra pode ser expressa de forma simples por:

$$
t_c\ge T_H \Rightarrow S_{novo}=S_{candidato}
$$

$$
t_c<T_H \Rightarrow S_{novo}=S_{anterior}
$$

onde:

- $S_{novo}$ é o estado resultante;
- $S_{candidato}$ é o novo estado detectado;
- $S_{anterior}$ é o estado mantido antes da confirmação.
...
Para limiares $H_1$ e $H_2$, a classificação conceitual pode ser escrita como:

$$
RH<H_1 \Rightarrow Estado_H=seco
$$

$$
H_1\le RH<H_2 \Rightarrow Estado_H=moderado
$$

$$
RH\ge H_2 \Rightarrow Estado_H=elevado
$$

onde $RH$ representa a umidade relativa.

Os valores de $H_1$ e $H_2$ devem ser aqueles efetivamente definidos no firmware.

------------------------------------------------------------------------
## 17. Estado de conforto

O estado de conforto combina informação ambiental para produzir uma interpretação mais direta ao usuário.
...
Quando a regra utiliza intervalos de temperatura e umidade, a condição de conforto pode ser expressa por:
...
Caso uma ou ambas não sejam satisfeitas:
...
Uma anomalia pode ser entendida como uma observação que viola regras ou padrões esperados.

Em um detector simples baseado em limites inferior e superior:

$$
x<L_{min} \Rightarrow A(x)=1
$$

$$
x>L_{max} \Rightarrow A(x)=1
$$

Se o valor permanecer dentro do intervalo esperado:

$$
L_{min}\le x\le L_{max} \Rightarrow A(x)=0
$$

onde:

- $A(x)=1$ representa condição anômala;
- $A(x)=0$ representa condição normal;
- $L_{min}$ é o limite inferior;
- $L_{max}$ é o limite superior.

O projeto pode futuramente substituir ou complementar regras fixas por modelos estatísticos ou TinyML.
